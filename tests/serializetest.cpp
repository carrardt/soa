...
int main(int argc, char* argv[])
{
	...
	auto in_arrays = soatl::make_field_arrays( soatl::cst::align<64>(), soatl::cst::chunk<8>(), e,atype,rx,mid,ry,rz );
	auto serialize_arrays = soatl::make_packed_field_arrays( soatl::cst::align<1>(), soatl::cst::chunk<1>(), e,atype,rx,mid,ry,rz );
	auto out_arrays = soatl::make_field_arrays( soatl::cst::align<64>(), soatl::cst::chunk<8>(), e,atype,rx,mid,ry,rz );
	...
	std::cout<<"memory usage = "<<serialize_arrays.data_size()<<std::endl;
...

	{
		...
		fout.write( (const char*) serialize_arrays.data() , serialize_arrays.data_size() );
	}
	serialize_arrays.resize(0);
	serialize_arrays.resize(N);
	{
		uint8_t* ptr = (uint8_t*) serialize_arrays.data();
		for(size_t i=0;i<serialize_arrays.data_size();i++) { ptr[i]=0; }
		std::ifstream fin("serialize.dat");
		fin.read( (char*) serialize_arrays.data() , serialize_arrays.data_size() );
	}
